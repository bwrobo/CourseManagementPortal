...
<?php
	...
	if (!empty($_POST)) {
	  ...
	  // Confirm Values are filled and in the proper format.
	    	if (Empty($firstName) or Empty($lastName) or Empty($email) or Empty($address) or Empty($ssn)) {
			    $err = "<br>All fields must be filled out before submitting.";
	    	}
			if (!isset($_SESSION['userID']) and empty($password1) and empty($password2) and empty($err)) {
				$err = "<br>All fields must be filled out before submitting.";
			}
			...
			If (!isset($_SESSION['userID']) and !Empty($username)) {
		...
		// If there are no errors, the user information is added to the database.
		    ...
		    if (empty($err) and empty($_SESSION['userID'])) {
			...
			elseif (empty($err) and !empty($_SESSION['userID']) and isset($_SESSION['userID'])) {
					...
					$result = mysqli_query($con,$sql);
					if (!empty($password1) and !empty($password2) and $password1==$password2) {
	...
	elseif(isset($_SESSION['userID']) and !empty($_SESSION['userID'])) {
